...
**요약**: 대표님이 스튜디오 목적 7개 정의 → 상단 탭 6개로 확정(사이드바 아닌 상단탭=휴대폰 대응). 메뉴명 확정: 올리기 / 성과표 / 호텔 / 키워드 / 전략 / 채널. 채널 필터는 메뉴 아래 공통. 원칙(부칙: 초등학생도 한눈에, 최대 원클릭). 앞으로 올릴 영상부터 조회수·클릭 집계(과거 영상 배제 — 링크 순서 TOP1/2/3 모름). 진행: [올리기]
...
## 🟡 P1 — [한국 호텔 분모 0] 아고다 한국어 파일에 국내 호텔이 0건 — 서울 4개 구·부산 등 성급 분포 분모가 비어 미개척을 못 잰다. 영어(EN) 파일에 있는지 확인해 채우기

**요약**: 우리 예약 88건 있는 시장. agoda_file_source 표의 EN 주소로 확인. 실측: 한국어 KO 파일 country='한국' 0건.
...
## ⚡ P1 — [관리자 호텔 상세 분석] 매니저 분석 미러링 + 수수료 노출 + 탭(개요·채널·패턴·예약상세)

**요약**: 인계서(2026-06-02) 다음작업. Phase1(본구현) 완료, Phase2~4 잔여. v_admin_* 뷰는 직전 채팅에서 신설됨(수수료 포함). 비admin은 매니저뷰로 폴백되어 수수료 비노출 유지. | 라벨 확정: '마케팅전'(대표님 2026-06-02). ④는 hotel-info.html에 기수집 확인되어 done. 잔여=②회차테이블신설+기간4구

- **자율성**: 🟡 SEMI
- **예상 시간**: 미정시간
- **카테고리**: feature
- **상태**: in_progress
- **ID**: `BL-ADMIN-HOTEL-DETAIL` (출처: handoff:2026-06-02)
...
## 🟡 P1 — [매출 회수] 승인됐는데 결제 안 한 호텔 푸시 화면 — 메일 발송 + 발송기록

**요약**: 사업가 결정 필요: 푸시 시점 며칠 기준? Aurora 디자인 통일.

- **자율성**: 🟡 SEMI
- **예상 시간**: 미정시간
- **카테고리**: feature
- **상태**: pending
- **ID**: `BL-PAYMENT-FOLLOWUP-CONSOLE` (출처: chat:2026-05-30 카테고리 전수 점검 (대표님 직접 짚음))

---

## 🟡 P1 — [재계약 매출] D-Day 임박 호텔 재계약 권유 화면 — 6개월 성과 자동 이미지 + 메일 + 발송기록

**요약**: BL-INVOICE 시리즈가 끝났으니 재계약·매출 회수 사이클이 다음 매출 톱니. 'large' 사이즈 — 단계 6개.

- **자율성**: 🟡 SEMI
- **예상 시간**: 미정시간
- **카테고리**: feature
- **상태**: pending
- **ID**: `BL-RENEWAL-CONSOLE` (출처: chat:2026-05-30 카테고리 전수 점검 (대표님 직접 짚음))

---

## 🟢 P1 — [클릭 추적] gohotel.win/h/{hid}?p={rank}&v={videoId} 단축URL → 클릭 카운트 → 아고다 리다이렉트

**요약**: D-006 "사업 모델 핵심 부품". 앞으로 올릴 영상 설명란 링크를 이 형태로. 이미 올라간 영상은 못 바꿈. link_clicks 테이블 없음(신설 필요). 순위(TOP1/2/3)별로 분리 저장 — BL-TRACK-001 의 "TOP1 클릭 4배" 분석용. 성과표·호텔 메뉴의 클릭 칸이 이거 없으면 계속 빈칸. api/youtube.js 의 설명란 링크

- **자율성**: 🟢 AUTO
- **예상 시간**: 4시간
- **카테고리**: ux
- **상태**: pending
- **ID**: `BL-TRACK-001-SHORTLINK` (출처: chat:2026-07-10 스튜디오 목적 7개 정의)
...
## 🟡 P2 — [지역 검색어가 거의 없음] 하카타 검색어 1개뿐 — 지역 단위로 따로 캐지 않고 도시 검색어에 지역명이 우연히 든 것만 쓴다(난바 17 vs 하카타 1). 지역 발굴 방식 결정 필요

**요약**: 지역까지 전부 캐면 도시당 수십 개 지역 × 자동완성 호출 = 429 위험. 후보: ①콘텐츠 만들 지역만 그때 캔다(지금도 [＋조사 추가]로 가능) ②상위 3~5개 지역만 자동. 대표님 판단 필요.

- **자율성**: 🟡 SEMI
- **예상 시간**: 미정시간
- **카테고리**: bugfix
- **상태**: pending
- **ID**: `BL-DISTRICT-KEYWORD` (출처: claude)
...
## 🟡 P2 — [매니저 페이지 예약 연동 보강] dashboard·marketing에 v_manager_recent_bookings 연동

- **자율성**: 🟡 SEMI
- **예상 시간**: 미정시간
- **카테고리**: manager-improvements
- **상태**: pending
- **ID**: `BL-MANAGER-PAGES-BOOKING-WIRE` (출처: auto_from_commit)

---

## 🟡 P2 — [메시지 통일] 매니저 검토중·거절 안내 메시지 — 한국어 정성스럽게, 통일 디자인

- **자율성**: 🟡 SEMI
- **예상 시간**: 미정시간
- **카테고리**: ux
- **상태**: pending
- **ID**: `BL-SALES-COPY-PENDING-REJECTED` (출처: chat:2026-05-30 카테고리 전수 점검 (대표님 직접 짚음))

---

## 🟡 P2 — [원클릭 운영] Dashboard 인박스 카드에서 바로 처리 — 다른 탭 이동 0번

**요약**: 의존: BL-PAYMENT-FOLLOWUP-CONSOLE + BL-RENEWAL-CONSOLE이 먼저 액션 API 갖춰야 함. 그 다음 인박스가 같은 API 호출.

- **자율성**: 🟡 SEMI
- **예상 시간**: 미정시간
- **카테고리**: feature
- **상태**: pending
- **ID**: `BL-INBOX-INLINE-ACTIONS` (출처: chat:2026-05-30 카테고리 전수 점검 (대표님 직접 짚음))

---

## 🟢 P2 — [매니저 퍼널 1단계 교체] 조회수 → 노출 횟수 (D-008 · D-040)

**요약**: tab-analytics 퍼널 4단계(영상 노출→Agoda 진입(CID)→예약 완료→매출 확정) 중 1·2단계가 빈 껍데기. videos 0행, link_clicks 테이블 없음. D-008: 조회수는 보조 지표 — 낮으면 "안 알려졌다"로 읽혀 매니저 이탈 유발. D-040: $200 = 영상 N개 누적·6개월 분산 노출. 따라서 1단계를 조회수가 아니라

- **자율성**: 🟢 AUTO
- **예상 시간**: 4시간
- **카테고리**: ux
- **상태**: pending
- **ID**: `BL-MGR-FUNNEL-EXPOSURE` (출처: chat:2026-07-10 유튜브 자동화 + 예약 적재 전수 점검)

---

## 🟢 P2 — [스티키 확장] analytics 외 나머지 10개 탭도 스크롤 시 메뉴 유지

**요약**: BL-ADMIN-ANALYTICS-STICKY-MENU(done)는 analytics 탭 8서브탭만 처리. .ad-topbar 가 position:relative 라 dashboard·bookings·hotels 등 나머지 10탭에서 스크롤 시 상단 메뉴가 사라짐. overflow:hidden 9곳 점검 포함. 탭 카테고리 재편은 SQ-F(blocked) 

- **자율성**: 🟢 AUTO
- **예상 시간**: 4시간
- **카테고리**: ux
- **상태**: pending
- **ID**: `BL-ADMIN-STICKY-MENU-ALL` (출처: chat:2026-07-10 유튜브 자동화 + 예약 적재 전수 점검)

---

## 🟢 P2 — [조회수 수집] 유튜브 Data API 로 주 1회 view_count 수집 (무료)

**요약**: videos.youtube_video_id 50개씩 묶어 videos.list 호출 = 1 unit/호출, 하루 10000 unit 무료. 주 1회면 사실상 0. publications 에 있는 영상만(앞으로 올릴 것부터). 스튜디오·관리자엔 표시, 매니저엔 미표시(D-008). vercel.json cron 주 1회. YOUTUBE_API_KEY env 
...
## 🟡 P3 — [도쿄 긴자 5성만 우리>전체] 지역 중심에서 800m 밖에 있는 우리 호텔이 분모에 안 잡힘 — 전 지역 중 1곳뿐이나 원인 확인 필요

**요약**: v_district_star 반경 0.8km. 긴자 5성 우리 8 vs 전체 6. 반경을 넓히면 옆 동네가 섞이므로 신중히.

- **자율성**: 🟡 SEMI
- **예상 시간**: 미정시간
- **카테고리**: bugfix
- **상태**: pending
- **ID**: `BL-GINZA-RADIUS` (출처: claude)
...
## 🟡 P3 — [운영 대시보드 본격 구축] admin.html을 서비스 운영의 단일 진입점으로
...
## 🟡 P3 — [기록] 2026-05-30 카테고리 전수 점검 결과 — chat-log 박기

**요약**: 자동화 가능 — 이 채팅 흐름 그대로 박기.

- **자율성**: 🟡 SEMI
- **예상 시간**: 미정시간
- **카테고리**: docs
- **상태**: pending
- **ID**: `BL-ADMIN-CATEGORY-AUDIT-CHATLOG` (출처: chat:2026-05-30 카테고리 전수 점검 (대표님 직접 짚음))

---

## ⚡ P3 — [드라이브 폴더 자동 처리] 대기 폴더에 원고를 넣으면 30분마다 자동 패키지 생성

**요약**: studio.html 업로드로 급한 불은 껐다(BL-YT-DOCX-INGEST done). 드라이브는 편의 기능. 주의: 서비스 계정은 저장 용량이 없어 파일을 못 만든다(구글 공식). 읽기·이동만 가능 → 패키지 txt 생성 불가. 대표님 계정으로 전용 폴더 신설 후 그 폴더만 서비스 계정에 공유. 기존 「호텔 콘텐츠」 폴더(소유자 grapio.do@gm

- **자율성**: 🟡 SEMI
- **예상 시간**: 5시간
- **카테고리**: infra
- **상태**: in_progress
- **막힘 사유**: 대표님이 서비스 계정 JSON 키를 Vercel env 에 넣어야 시작 가능
- **ID**: `BL-YT-DRIVE-WATCH` (출처: chat:2026-07-10)
