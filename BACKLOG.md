...
### Issue #4: Page Gallery iframe 미리보기에서 로그인 화면이 표시됨 (2026-04-29 발견 / 동일자 부분 해결)
**현상**: admin-gallery.html에서 로그인 필요 페이지(매니저 대시보드, 호텔 정보 등록, 예약 분석 등)의 "라이브 미리보기" 클릭 시 iframe 안에 로그인 화면이 노출됨.

**원인**: same-origin iframe이라 localStorage는 공유되지만, Supabase auth 클라이언트가 iframe 컨텍스트에서 세션을 즉시 hydrate하지 않거나, iframe sandbox 옵션 또는 cross-context auth 동작 차이로 인해 미인증으로 판단 → login.html로 redirect.

**부분 해결 (2026-04-29 5차 커밋)**: GitHub Actions 자동 캡처 워크플로 신설. 매니저/어드민 페이지를 매니저/어드민 계정으로 로그인 후 정적 PNG 캡처 → admin-gallery에 정적 썸네일 표시. iframe 미리보기 의존 제거.

**남은 옵션 (P3로 강등)**: iframe 자체를 살리고 싶으면 postMessage + setSession / sandbox 옵션 / flowType 점검. 정적 캡처가 잘 작동하면 iframe 미리보기 기능 자체를 제거하는 것도 검토 가능.

**대표님 복귀 후 액션**: GitHub Secrets 4개 등록 (CHANGELOG 5차 항목 표 참조) → Actions에서 수동 실행.

**관련 파일**: `admin-gallery.html`, `scripts/capture-pages.mjs`, `scripts/pages-meta.mjs`, `.github/workflows/capture-pages.yml`
