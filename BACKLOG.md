...
## ⚡ P0 — Claude 추천 다시 받기 버튼 폐기 — 자동 채움과 100% 중복 (D-019 정신)

**요약**: 직전 BL-DECISION-AUTOFILL에서 그림 일치 시 자동 채움 박았는데 추천 버튼이 여전히 존재. 자동 채움 + 추천 버튼 = 같은 동작 (마지막 Claude 답을 textarea에). D-019 중복 3중 정리 정신 위반.

fix: grim-suggest-btn HTML + 핸들러 + CSS 모두 제거. 사용자가 textarea 직접 수정해서 

- **자율성**: 🟡 SEMI
- **예상 시간**: 미정시간
- **카테고리**: fix
- **상태**: in_progress
- **ID**: `BL-SUGGEST-BTN-RETIRE` (출처: 대표님 직접 보고 (2026-05-11) — 추천 다시 받기 버튼 필요하나? 자동으로 채워졌는데)
