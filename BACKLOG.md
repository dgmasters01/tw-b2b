...
## ⚡ P0 — 페이지 지도 ↔ 결정 대기 ↔ 자율 큐 3방향 클릭 연동 + BL-페이지 매핑 보강

**요약**: 대표님 본질: 페이지 지도 / 결정 대기 / 자율 큐 3개 카드가 따로따로 작동. 같은 BL이 어디 묶이는지, 어떤 페이지에 들어가는지 시각적으로 연결 안 됨.

fix 2가지:
1) PAGE_INFO 매핑 보강 — 자율 큐 12건 모두 페이지 매핑
2) 3방향 클릭 연동:
   - 페이지 카드 클릭 → 묶인 BL이 결정 대기 + 자율 큐에서 하이라이트
...
- **카테고리**: feature
- **상태**: in_progress
- **ID**: `BL-3WAY-LINK-CARDS` (출처: 대표님 직접 보고 (2026-05-11) — 카드 만들었는데 연동이 안 됨. 어떻게 하면 전체와 작업을 쉽게 연동해서 추천 순서대로 파악하면서 작업 지시할 수 있을까?)
