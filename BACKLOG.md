...
## ⚡ P0 — 결정 영향력 카드 — 어떤 결정 박으면 어떤 BL/페이지가 풀리는지 한눈에

**요약**: 대표님 본질 고민:
1. 페이지 골격이 먼저, 기능은 그 위 — 어디 노출/연동될지도 모르고 기능 먼저 만드는 게 말이 안 됨
2. 결정 24개 중 무엇부터? 의존성 트리가 안 보임
3. 긴급 결정 다 박으면 새 작업이 자동 생성되나? 시스템이 알려줘야 함

fix: admin-status에 "결정 영향력 카드" 신설. 각 결정마다 박혀있을 때 풀리는 BL

- **자율성**: 🟡 SEMI
- **예상 시간**: 미정시간
- **카테고리**: feature
- **상태**: in_progress
- **ID**: `BL-DECISION-IMPACT-CARD` (출처: 대표님 직접 보고 (2026-05-11) — 결정 24개를 무엇부터 박아야 페이지가 만들어지고 기능 순서가 정해지는지 모르겠다. 알려주는 게 있어야 하지 않나)
