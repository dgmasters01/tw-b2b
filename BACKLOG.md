...
## 🟡 P1 — [admin 사이드바] 만들어둔 페이지 진입로 누락 — 청구서·매니저허브·매니저 시점 보기 메뉴 추가

**요약**: 기존 페이지 그대로 활용. admin.html 사이드바만 추가. 거대파일이지만 nav 영역 str_replace 1~2회로 끝.

- **자율성**: 🟡 SEMI
- **예상 시간**: 미정시간
- **카테고리**: admin
- **상태**: pending
- **ID**: `BL-ADMIN-SIDEBAR-MISSING-ENTRIES` (출처: chat:2026-05-30 카테고리 전수 점검 (대표님 직접 짚음))

---

## 🟡 P1 — [보안 정비] 로그인 유지 체크박스 — 무조건 영구 자동로그인 → 사용자 선택

**요약**: Supabase auth 공식 가이드: { auth: { storage: customStorage, persistSession: true } } 패턴. customStorage가 localStorage 또는 sessionStorage 가리키게.

- **자율성**: 🟡 SEMI
- **예상 시간**: 미정시간
- **카테고리**: security
- **상태**: pending
- **ID**: `BL-LOGIN-PERSIST-OPTIN` (출처: chat:2026-05-30 카테고리 전수 점검 (대표님 직접 짚음))
...
## 🟡 P2 — [영상 게시 입력] 게시 완료 도장만 찍는 게 아니라 채널별 영상 URL·게시일·썸네일 입력
