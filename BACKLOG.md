...
## 🟡 P1 — [영상 게시 입력] 게시 완료 도장만 찍는 게 아니라 채널별 영상 URL·게시일·썸네일 입력

**요약**:  | [2026-07-10 범위 확장] 에디터 전용 studio.html 을 이 BL 에 흡수(신규 BL 만들지 않음). api/youtube.js 산출물(제목·설명란·키워드·해시태그·파일명·챕터)을 publications 에 저장 → studio.html 이 복사 버튼으로 제공 → 유튜브 URL 입력 시 status draft→published + vid

- **자율성**: 🟡 SEMI
- **예상 시간**: 미정시간
- **카테고리**: feature
- **상태**: pending
- **ID**: `BL-VIDEO-PUBLISH-INPUT-MODAL` (출처: chat:2026-05-30 카테고리 전수 점검 (대표님 직접 짚음))

---

## 🟢 P1 — [권한 후속] staff 가 is_admin() 을 통과 — 콘텐츠 업로드 전담 editor 등급 분리

**요약**: 현행 is_admin(uid) = role IN (owner, admin, staff). staff 를 주면 admin.html 전체(환불·매출·호텔 승인)가 열림. 스리랑카 콘텐츠 담당자에게 줄 수 없음. editor 역할 신설 + is_editor() 신설, is_admin() 에서 editor 제외. editor 는 publications/videos

- **자율성**: 🟢 AUTO
- **예상 시간**: 4시간
- **카테고리**: security
- **상태**: pending
- **ID**: `BL-EDITOR-ROLE-SPLIT` (출처: chat:2026-07-10 유튜브 자동화 + 예약 적재 전수 점검)
