...
## ⚡ P0 — 다음추천 큐가 결정 미박힘 작업을 자율로 잘못 분류 — 결정 선행 가드 강화

**요약**: 결함 본질: admin-status taskRole() 함수의 결정 선행 가드가 AND 조건(decs.length>0 && !claude_can_auto)으로 잘못 작성됨. BL-TRACK-001 같은 claude_can_auto=true 작업은 requires_decisions_first가 박혀있어도 ceo 분류 안 되고 auto 큐로 넘어감.
...
- **카테고리**: fix
- **상태**: in_progress
- **ID**: `BL-DECISION-GATE-IN-QUEUE` (출처: 대표님 직접 보고 (2026-05-11) — 다음추천이 BL-TRACK-001로 떠 있는데, 결정 안 박힌 사업 시스템 작업이 자율 큐에 무작정 박혀있는 사고)
