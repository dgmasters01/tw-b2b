...
## 2026-05-03 - Phase 2-B-2.5

- [변경사유: 외근 중 작업 위임 시 타이핑 부담 95% 감소, Phase 2-B-4 외근모드 자동화의 사용자 트리거 UX 정비] admin-tasks.html: Claude Code 원클릭 위임 기능 추가 (모달 내 "🚀 Claude에 위임" 버튼 + navigator.clipboard.writeText로 작업 정보+실행 지침 자동 복사 + 토스트 알림 "📋 클립보드 복사 완료". autonomous=활성/외근모드 시 강조, human=비활성+안내). 백업: admin-tasks.html.bak_20260503_phase2b25
...
### Added
- 작업관리 시스템 1단계: admin-tasks.html, tasks.json (51개 작업), 통합 빌더/스냅샷/역방향 빌더 스크립트
- tasks_snapshots/ 백업 시스템
- HANDOFF_TASK_MGMT.md 인계 문서

[변경사유] 1단계 작업물 라이브 배포 누락분 복구 (2026-05-02 인계 문서 검증 결과)
...
### 변경 파일
- `admin.html`: BKA 예약 패턴 영역 — `dayDrillHt` 본체 교체 (5개 키 세팅) + `rDayHotels`의 `if(DS.dayHt){...}` 분기 통째 제거

### 사업 결정 (대표님 직접 확정)
1. **호텔 클릭 동작 통일**: Analytics → 예약 패턴 → 캘린더 일자 클릭 → 일자별 표 → 호텔 클릭 시, "2024-06 나라별 (전체) → 클릭 → 도시 → 호텔" 메인 흐름과 동일하게 호텔 엔딩 페이지로 이동. 24차의 인라인 호텔 상세 펼침은 폐기. 캘린더 토글 + 나라 인라인 펼침은 유지.
2. **수수료 컬럼**: admin.html 호텔 엔딩 페이지의 수수료 컬럼은 그대로 유지 (관리자는 통합 관리 목적이므로 수수료 노출 필요). 추후 호텔매니저 전용 페이지에서는 수수료 미노출 — 매니저 페이지는 25차 범위 외.
3. **호텔 소개 #hi-card**: 호텔 엔딩 페이지의 `#hi-card` 영역이 `hotel-info.html`에서 매니저가 회원가입 후 입력한 정보(주소/전화/이메일/웹사이트/평점/영업담당자)를 hotels 테이블 통해 자동 동기화. 코드 변경 없이 동작 확인만 필요.

### 문제
- 24차 일자별 인라인 드릴다운에서 호텔 행 클릭 시, 같은 카드 안에 그 호텔의 해당 날짜 예약 1건만 펼치는 형태였음 (`DS.dayHt` 키 사용).
- 결과적으로 "예약 패턴에서 호텔 클릭"이 두 흐름으로 분기됨:
  - (A) 패턴 메인 분기 → `DS.patHt`/`DS.patCi`/`DS.patCo` → `rHtD()` 호텔 엔딩 페이지 (호텔 정보 + 전체 예약 + 매니저 액션 + #hi-card 호텔 소개 + hotel-info 동기화)
  - (B) 일자별 인라인 분기 → `DS.dayHt` → 그 날짜 한정 1건짜리 표만 (정보 부족, 매니저 동선 단절)

### 원인
- 24차 설계 시 일자별 컨텍스트 보존을 위해 별도 키 `DS.dayHt` 분기를 두었으나, 실사용에서 매니저는 호텔의 풀 컨텍스트(전체 예약 + 호텔 소개 + 매니저 액션)를 원하는 동선으로 확인됨.
- 또한 `DS.patCi`(도시) 키가 누락되면 호텔 엔딩 페이지의 브레드크럼/메인 분기 일치성이 깨지므로 5개 키 세팅 필요.

### 해결
1. **`dayDrillHt(h, ci, c)` 본체 교체** — 패턴 메인 분기와 동일한 키 5개 세팅:
   - `DS.patHt = { h, ci, co }`
   - `DS.patCo = c`
   - `DS.patYM = calYM`
   - `DS.patCi = ci` ← 도시 키. 메인 분기 "나라→도시→호텔" 흐름과 정확히 일치시키기 위해 필수
   - `DS.dayHt = null` (잔존 인라인 상태 명시 초기화)
2. **`rDayHotels()`의 `if(DS.dayHt){...}` 분기 통째 제거** — 호텔 엔딩 페이지로 점프하므로 인라인 호텔 상세 단계 불필요. 호텔 목록 단계만 남김.
3. **결과 흐름**: 일자 선택 → 나라 클릭 (인라인 호텔 목록 펼침, 캘린더+일자별 표 유지) → 호텔 클릭 → `rPa()` 메인 분기의 `DS.patHt`/`DS.patCi`/`DS.patCo` 경로를 타고 **`rHtD()` 호텔 엔딩 페이지**(호텔 소개 #hi-card + 전체 예약 + 매니저 액션 + 수수료 컬럼)로 진입. 패턴 메인 분기와 정확히 동일한 도착지.

### 검증 (자동 4단계 모두 PASS)
1. **JS 문법**: 인라인 script 전체 `new Function()` 생성 PASS
2. **함수/DOM 보존**: `toggleCalDay`, `dayDrillCo`, `dayDrillHt`, `rDayHotels`, `rPa`, `rHtD`, `fillHotelInfo`, `#hi-card` 모두 보존
3. **호출부 보존**: 캘린더 셀 onclick=toggleCalDay / 일자별 나라행 onclick=dayDrillCo / 일자별 호텔행 onclick=dayDrillHt 모두 보존
4. **시뮬레이션**: `DS.dayHt = {...}` 객체 세팅 건수 = 0건 (구 분기 진입 불가능 확정), `rDayHotels` 내 `if(DS.dayHt)` 분기 부재, `dayDrillHt` 본체 5개 키 모두 OK

### 백업
- `_backup_20260502/admin.html` — push 직전 d239339 (24차) 원본 그대로 보존
...
### 변경 파일
- `admin.html`: BKA 예약 패턴 영역 — 캘린더 셀 onclick 교체 + 일자별 표 onclick 교체 + 신규 함수 3개 추가/노출 (총 +60줄)

### 문제
- 6월 4일 캘린더 셀을 두 번째 클릭하면 (정상: 선택 해제) 화면이 베트남 도시 목록으로 점프됨.
- 일자별 표에서 Vietnam 같은 나라 행 클릭 시, 캘린더가 사라지고 URL이 `#pattern/{"patCo":"Vietnam"...}`로 변경되며 매니저 영업 동선이 끊김.

### 원인
- 캘린더 일자 셀 onclick: `"calDay='+(calDay===d?'null':d)+';dc();rr()"` — 문자열 보간 중 'null'/숫자 분기가 인라인 onclick에 직접 박혀 있음. 클릭 위치/타이밍에 따라 의도치 않은 셀 클릭이 다음 라인의 일자별 표 행 클릭으로 이어지는 이벤트 흐름 발생.
- 일자별 표의 나라 행 onclick: `"DS.patCo=...;DS.patYM=calYM;dc();rr()"` — DS.patCo를 세팅하면 `rPa()`가 메인 분기(3934줄)로 빠지면서 캘린더와 일자별 표가 사라지고 도시 목록 페이지로 강제 점프됨. 사업적으로 매니저가 "그날 그 나라에 어떤 호텔이 예약되었는지"를 한눈에 보려는 동선과 충돌.

### 해결
1. **캘린더 셀**: onclick을 `toggleCalDay(d)` 호출로 교체. 같은 일자 두 번째 클릭 시 명확하게 `calDay=null`로 토글. `event.stopPropagation()`으로 이벤트 버블링 차단.
2. **일자별 표 나라 행**: onclick을 `dayDrillCo(co)` 호출로 교체. `DS.patCo`는 절대 건드리지 않고 별도 키 `DS.dayCo`만 사용 — 메인 분기와 완전 분리.
3. **신규 흐름**: 일자 선택 → 나라 클릭 → **캘린더+일자별 표는 그대로 유지**된 상태에서 **그 나라의 호텔 목록이 같은 카드 안에 인라인으로 펼쳐짐** → 호텔 클릭 시 그 호텔의 그 날 예약 상세까지 인라인 펼침. 다른 나라 클릭 시 펼침 자동 전환. 다른 일자 클릭 시 인라인 상태 자동 초기화.
4. **신규 함수 3개** (BKA IIFE 내부 정의 + global 노출):
   - `toggleCalDay(d)` — 캘린더 일자 토글
   - `dayDrillCo(co)` — 일자별 나라 인라인 드릴다운
   - `dayDrillHt(h, ci, co)` — 일자별 호텔 인라인 드릴다운
   - `rDayHotels()` — 인라인 펼침 영역 HTML 렌더 (DS.dayCo 없으면 빈 문자열)

### 자동 검증
- 7개 script 블록 문법 OK / inline 핸들러 매칭 OK (toggleCalDay/dayDrillCo/dayDrillHt 노출 확인) / setActiveTab→BKA.mount OK / DOM 컨테이너 OK.
- 동작 시나리오 8건 시뮬레이션 통과: ①일자 단일 클릭 ②일자 토글 해제 ③나라 인라인 펼침 ④호텔 인라인 펼침 ⑤나라 펼침 해제 ⑥다른 일자로 전환 시 dayCo 자동 초기화 ⑦DS.patCo 미간섭 확인 ⑧URL 변화 없음 확인.

### 영향 범위
- 예약 패턴 탭의 일자→나라→호텔 동선이 매니저 영업 직관에 맞게 단일 화면 인라인 펼침으로 통합.
- 기존 "나라별 (전체)" 표(같은 화면 하단)에서 나라 클릭 시 동작은 변경 없음 — 그대로 메인 분기로 이동 (대표님이 명시적으로 "다른 페이지로 가는" 흐름을 선택할 수 있도록 유지).
- 다른 탭(전체 현황/채널별/도시별 등) 영향 없음.
