...
⑥ **작업 단위 1~2개로 작게 + 끊김 객관 트리거 4종 (D-049)** — 토큰 한계 위험 감지 시 부칙 13에 따라 새 채팅 권장. **객관 트리거(판단 개입 0%, 카운트만 봄):** ⓐ 응답 15회 초과 → "🚨 끊김 위험 N회" 박음 ⓑ str_replace+create_file 누적 10회 초과 → 동일 경고 ⓒ `[step:done:N]` commit마다 → "끊고/계속" 1줄 질문 ⓓ 1500줄+ 파일 수정 시 → 그 작업 1개만 한 채팅. 디테일: 헌법 부칙 16.1.
