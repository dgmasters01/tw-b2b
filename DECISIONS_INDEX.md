...
|---|---|:---:|:---:|:---:|---|
| D-118 | **검색 엔진을 «점수제 + 검색 전용 표»로 교체** — 대표님: *«검색 기능의 정확성이 떨어짐»* · *«띄어쓰기 없이 붙여서 적으면 검색이 안 되는 거 아닌가»* · *«영어로 적으면 또 검색이 안 되는 거 아니야»* · *«글자를 적으니깐 한참 뒤에 비슷한 글자가 밑에 뜬다»* · *«글로벌 서비스의 기준을 찾아서 우리도 엔진을 어떻게 할 건지 방법을 찾아»*. 🔴 **원인 둘을 코드에서 확정** ①남은 말 중 **첫 단어 하나만** 썼다(`p.rest[0]`) — 「그랜드 센트럴」이 「그랜드」가 됐다 ②정렬에 **일치도가 아예 없었다**(소개 날짜·값 순) — 정확히 맞은 1곳이 33곳 사이에서 **21번째**. 정한 것 — **창고 안에서 끝낸다**(💰 Algolia·Elasticsearch 금지 · 월 0원). ①정규화 열쇠 4벌을 **미리** 만들어 두는 검색 전용 표 `shop_hotel_search`(4,776줄 = 소개 1,071 + 신상 3,705) ②`pg_trgm` GIN 색인 ③점수판 **100 같다 / 80 그 말로 시작 / 70 친 말이 통째로 / 60 모든 말이 다 / 30 일부만 / 10~25 오타** — 🔴 **한글로 치든 영어로 치든 같은 점수판**, 점수를 **먼저** 보고 동점일 때만 후기 수·값 ④창구는 창고를 **한 번**만 부른다(옛 6번). 🔴 **대상을 넓히지 않는다** — 우리가 소개한 호텔 + 신상만. 아고다 명부 23,050곳을 넣으면 값을 재지 않는 호텔이 섞여 「가장 싼 날을 찾아 드립니다」가 깨진다. 🔴 **기존 `v_shop_card` 를 고치지 않고 가벼운 판(`v_shop_card_lite`)을 따로 만들었다** — 다른 화면 다섯 곳이 그 뷰를 쓴다. 되돌릴 때 건드릴 것이 없다. **실측 전→후**: 「그랜드 센트럴 호텔 상하이」 21번째·4.34초 → **1등** · 「그랜드센트럴」(붙여씀) 0곳 → **1등** · 「hilton danang」 1등이 발리 → **힐튼 다낭** · 「Grand Central Hotel Shanghai」 1등이 그랜드 시티텔 하노이 → **정답** · 「일본 10만원 3성급」 8.45초 → 정상 · 속도 **4.34초 → 약 0.2초(20배)**. 🔴 **속도의 절반은 「함수가 도는 자리」였다** — 창구는 미국(iad1), 창고는 도쿄. 조회마다 태평양을 왕복했다. `vercel.json` 두 줄로 도쿄(hnd1)로 옮겼다(**요금 0원** — Hobby 요금제도 단일 지역 선택 가능, 공식 문서 확인). 🔴 **점수 함수 안에 «작은 질의»를 두면 창고가 줄마다 그것을 따로 돌린다** — 열쇠 하나 90ms·넷 346ms → 무늬를 밖에서 한 번만 만들어 넘기니 **12.9ms(27배)**. 🔴 **「신라스테이」는 우리 창고에 없다** — 검색 고장이 아니라 자료가 없는 것(눌러서 확인). 있는 「신라」는 `신라 모노그램 다낭` 하나뿐이고 새 엔진은 그것을 1등으로 낸다. **없는 호텔을 있는 척하지 않는다.** 2차(초성 `ㅅㄹㅅㅌㅇ` · 한/영 자판 `rmfosem` · 「혹시 이것을 찾으셨나요?」)는 **1차를 지켜본 뒤에** 따로 넣는다 | ux | 2026-09-09 | 확정 | _business/decisions/2026-09-09-shop-search-engine.md, _business/shop/SEARCH_REBUILD.md, _business/shop/SHOP_TECH.md §40·§41·§42·§43, _business/shop/SHOP_ROLLBACK.md §27, travelwinners-shop: sql/2026-09-09-search-engine.sql, api/search.js, api/cron/learn.js, public/shop.js, vercel.json |
| D-117 | **신상 호텔 섹션 + 공유 페이지** — 대표님: *«새호텔이라고 이야기 하면 사람들이 우선적으로 클릭해서 예약할 확률이 높잖아… 우리는 수익을 만들어 내야지»* · *«오픈, 재단장 이렇게 가자»*. 「신상」 = **2024년 이후 오픈 또는 재단장** — 개관연도만 쓰면 1,058곳인데 **재단장을 함께 쓰면 2,160곳**(두 배). 딱지 「2025 오픈」/「2025 새단장」 둘로 나눔. 🔴 **연도를 모르는 호텔에는 신상 딱지를 붙이지 않는다**(후보의 60%가 빈칸 — 손님이 가서 낡은 호텔을 만나면 안 돌아온다. 아는 것만으로 전 세계 12,701곳). 🔴 **개관연도를 아고다에 묻지 않았다** — 한국어 명부(39칸)에 `yearopened`·`yearrenovated` 가 이미 있었다. 계획했던 **호출 5,700회·사흘 → 0회·43초**, 상위 300곳 한정도 불필요해져 **20,958곳 전량** 채움. 도시 범위 19개 → **우리 페이지가 있는 58개 도시 전체**(비용 0원) → 신상 **3,676곳·19개국 46개 도시**, 10곳 미만 도시는 화면에서 자동 제외. 화면은 **나라별 개별 섹션**(제목 「{나라} 최근 오픈, 새단장 추천 숙소」 → 도시 탭 → 가로 10개 → 더보기) 7개 나라. 평점·후기 제거, **성급은 사진 안 오른쪽 아래**. 🔴 **사진 규격 480→320/1200→800/2000 세 번 변경 사고** — 매번 «용량 절약» 기준으로 정한 것이 원인이고 실제 절약액은 **0원**이었다. **정본: 사진은 아끼는 대상이 아니라 팔리는 물건이다. 규격은 «화면에 그려지는 크기의 3배»로 정한다.** 아고다는 WebP 를 주지 않음(「1,600px WebP」 폐기) · PNG 467장 JPEG 변환(7.3MB→495KB) · `photo_spec` 칸으로 재수집 방지. 🔴 **아고다 한도 2,160 은 아고다가 아니라 «우리가» 정한 추정값**(공식 문서에 숫자 0건 · 403 만 존재 · 요금표 없음). 진짜 병목은 **크론 1회/시간 × 날짜 3개 = 시간당 54회**, 300초 중 50초만 사용. → 크론 10분 · 동시 4묶음 · 신상 편입 후 **호출 21→104회에도 응답 전부 200 · 차단 0건**. 샵 열쇠 cid **1919025**(스튜디오 1972105와 다름) 실측 확인. 가격 표시는 **다섯 줄 자리를 그대로 두고 비교 대상만 교체**(신상 = 가장 비싼 날 취소선 → 가장 싼 값 → 「29,788원 더 싸게 예약」). 🔴 **차액이 작아도 표시**(대표님: *«1000원짜리도 아끼는 시대»*) · 값 3일치 미만이면 「가장」 표현 금지. 공유 페이지 **`/h/{번호}` 를 새로** 만든다(`/hotel/{번호}` 는 손대지 않음 — 되돌릴 때 공유 버튼 한 줄). 🔴 **카톡 카드는 카톡이 저장해 며칠간 안 바뀌므로 가격에 «9월 6일 조회» 날짜를 붙인다**. 「숙소 둘러 보기」 = 같은 도시·**가격 ±30%**·지금 가장 많이 싸진 순 10곳(매일 순서가 바뀜) · 추천 상품은 **도시별 맞춤**(1,097개/59개 도시, 없는 도시는 칸 숨김) · 클릭에 `src=share`. **배너 자리 2곳** — 예약 버튼 아래 **이벤트**(눈이 가장 오래 머무는 자리), 추천 상품 아래 **광고**. 기존 `shop_banner` 표 사용·기간 지나면 자동 하강·0장이면 칸 소멸(D-113 자동회전 금지 유지). 🔴 **「여행능력자들이 직접 자보고 매긴 순위입니다」 문구 전면 폐기** — 대표님: *«절대 넣지마. 맞지 않는거야»*. 신상 3,676곳은 가본 적이 없어 거짓이 된다 → **C-29 도 화면에서 내림**. 브랜드 표기 **「여행능력자들」 한 가지**(손님 화면 6곳에서 SHOP 제거·잔재 0 실측 · 파일명·도메인은 유지). 도메인은 추후 `www.travelwinners.shop`(옛 서버 살아 있음)와 `여행능력자들.shop`(=`xn--9h1bq6bn6dbxuc3be98a.shop` · **현재 503**) 둘 다 이전 — 🔴 **죽어 있는 한글부터**, 대표 주소는 영문 권장 | ux | 2026-09-06 | 확정 | _business/decisions/2026-09-06-shop-new-hotels-and-share-page.md, _business/shop/SHOP_TECH.md §6-A·§6-B, _business/shop/SHOP_ROLLBACK.md, api/new.js, public/new.html, public/shop.js, api/cron/collect.js |
...
| D-118 | 2026-09-09 (상세=_business/decisions/2026-09-09-shop-search-engine.md) | 검색 엔진 점수제 교체 (4.34초·21번째 → 0.2초·1등) |
| D-117 | 2026-09-06 (상세=_business/decisions/2026-09-06-shop-new-hotels-and-share-page.md) | 신상 호텔(오픈·재단장) · 사진 규격 정본 · 아고다 한도 실측 · 공유 페이지 |
...
| 날짜 | 변경 내용 |
|---|---|
| 2026-09-09 | D-118 등록 (검색 엔진 점수제 교체 · 검색 전용 표 4,776줄 · 함수 실행 지역을 도쿄로 — 4.34초 → 약 0.2초) |
| 2026-09-06 | D-117 등록 (신상 호텔 3,676곳 · 사진 800/2000 정본화 · 아고다 한도가 «우리 추정값»임을 실측 · 「직접 자보고 매긴 순위」 문구 폐기) |
