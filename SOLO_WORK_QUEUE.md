...
### I. 🟢 ✅ **[DONE 2026-04-30 — 13차]** AUTO — `api/lib/` → `api/_lib/` 후속 정리
**완료 내용**: vercel.json 수정 대신 `api/_lib/README.md` 신설로 디렉토리 정책 명문화. underscore-prefix 자동 제외 + `api/*.js` 단일 레벨 와일드카드로 이미 이중 안전망 작동 중이라는 점, 신규 헬퍼는 반드시 `api/_lib/`에 추가해야 한다는 점, 현재 함수 카운트 9/12와 통합 라우터 패턴 권장을 함께 문서화. 코드 변경 없음 → 회귀 위험 0.
