...
### H. 🟢 AUTO — [권한 후속] staff 가 is_admin() 을 통과 — 콘텐츠 업로드 전담 editor 등급 분리

**ID**: `BL-EDITOR-ROLE-SPLIT`  
**카테고리**: security  
**예상 시간**: 4시간  

**메모**: 현행 is_admin(uid) = role IN (owner, admin, staff). staff 를 주면 admin.html 전체(환불·매출·호텔 승인)가 열림. 스리랑카 콘텐츠 담당자에게 줄 수 없음. editor 역할 신설 + is_editor() 신설, is_admin() 에서 editor 제외. editor 는 publications/videos 만 접근, bookings·payments 접근 0. BL-ADMIN-AUTH-V2(done) 의 5단계 위에 얹는 후속.
...
**메모**: 인계서(2026-06-02) 다음작업. Phase1(본구현) 완료, Phase2~4 잔여. v_admin_* 뷰는 직전 채팅에서 신설됨(수수료 포함). 비admin은 매니저뷰로 폴백되어 수수료 비노출 유지. | 라벨 확정: '마케팅전'(대표님 2026-06-02). ④는 hotel-info.html에 기수집 확인되어 done. 잔여=②회차테이블신설+기간4구분, ③마케팅전 예약 매칭(예약데이터 0건이라 데이터 충전 후 검증).
...
### E. 🟢 AUTO — [매니저 퍼널 1단계 교체] 조회수 → 노출 횟수 (D-008 · D-040)

**ID**: `BL-MGR-FUNNEL-EXPOSURE`  
**카테고리**: ux  
**예상 시간**: 4시간  

**메모**: tab-analytics 퍼널 4단계(영상 노출→Agoda 진입(CID)→예약 완료→매출 확정) 중 1·2단계가 빈 껍데기. videos 0행, link_clicks 테이블 없음. D-008: 조회수는 보조 지표 — 낮으면 "안 알려졌다"로 읽혀 매니저 이탈 유발. D-040: $200 = 영상 N개 누적·6개월 분산 노출. 따라서 1단계를 조회수가 아니라 노출 횟수(영상 N편 × 채널 M개)로 교체. 2단계는 BL-TRACK-001(단축 URL 클릭 카운트)이 채움. 퍼널 자체는 제거하지 않음 — D-006 "사업 모델 핵심 부품

---

### F. 🟢 AUTO — [스티키 확장] analytics 외 나머지 10개 탭도 스크롤 시 메뉴 유지

**ID**: `BL-ADMIN-STICKY-MENU-ALL`  
**카테고리**: ux  
**예상 시간**: 4시간  

**메모**: BL-ADMIN-ANALYTICS-STICKY-MENU(done)는 analytics 탭 8서브탭만 처리. .ad-topbar 가 position:relative 라 dashboard·bookings·hotels 등 나머지 10탭에서 스크롤 시 상단 메뉴가 사라짐. overflow:hidden 9곳 점검 포함. 탭 카테고리 재편은 SQ-F(blocked) 소관 — 여기서 건드리지 않음.
...
### K. 🟡 SEMI — [admin Members 탭 국가별 필터] 동남아 그룹 강조
...
### L. 🟡 SEMI — [영수증 PDF 5년 영구 보관] Supabase + S3 백업
...
### M. 🟡 SEMI — [이벤트 사이트 고객 회원가입 + 마케팅 동의 DB] 고객 자산화
...
### N. 🟡 SEMI — [이벤트 사이트 호텔 대리 결제] Agoda affiliate 또는 직접 결제
...
### O. 🟡 SEMI — [이벤트 송출 호텔 알림 + admin 송출 관리 탭]
...
### P. 🟡 SEMI — [화면 라벨 정리] 사이드바 메뉴 사업 본질 맞춤 + 영한 토글 전수 점검
