...
### G. 🟢 AUTO — [원고 투입 경로] docx 4.5MB 초과 — 서버가 드라이브/스토리지에서 원고를 읽게

**ID**: `BL-YT-DOCX-INGEST`  
**카테고리**: infra  
**예상 시간**: 4시간  

**메모**: 원고 docx 는 이미지 때문에 3~4MB. base64 로 감싸면 Vercel serverless payload 한도(4.5MB)를 넘어 413. 지금은 패키지를 미리 만들어 넣는 경로만 동작. 해법 후보: ①구글 드라이브 폴더 감시(서비스 계정) ②Supabase Storage 업로드 후 서버가 읽기 ③브라우저에서 텍스트만 추출해 전송. 대표님 선호는 드라이브 폴더 자동 처리(3.작업대기 → 4.작업완료 → 5.확인필요).
