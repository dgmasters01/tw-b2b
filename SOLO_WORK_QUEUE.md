...
### B. 🟡 SEMI — 페이지 지도 ↔ 결정 대기 ↔ 자율 큐 3방향 클릭 연동 + BL-페이지 매핑 보강

**ID**: `BL-3WAY-LINK-CARDS`  
**카테고리**: feature  
**예상 시간**: 미정시간  

**메모**: 대표님 본질: 페이지 지도 / 결정 대기 / 자율 큐 3개 카드가 따로따로 작동. 같은 BL이 어디 묶이는지, 어떤 페이지에 들어가는지 시각적으로 연결 안 됨.

fix 2가지:
1) PAGE_INFO 매핑 보강 — 자율 큐 12건 모두 페이지 매핑
2) 3방향 클릭 연동:
   - 페이지 카드 클릭 → 묶인 BL이 결정 대기 + 자율 큐에서 하이라이트
   - BL 클릭 (결정 대기/자율 큐) → 묶인 페이지가 페이지 지도에서 하이라이트
   - 같은 하이라이트 상태 유지 + 한 번 더 클릭하면 해제
