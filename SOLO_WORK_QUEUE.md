...
### C. 🟡 SEMI — 핑퐁 끝나면 결정 한 줄 자동 채움 — 사용자 추가 작업 0

**ID**: `BL-DECISION-AUTOFILL`  
**카테고리**: fix  
**예상 시간**: 미정시간  

**메모**: 본질 결함: 핑퐁 라운드 9까지 다 돌리면 마지막 Claude 답이 이미 충분히 정리된 결정 본문임 (예: "a안으로 갑니다. 박을 것 3가지: 접속 로그, 실행 로그, 최근 활동 박스..."). 그런데 시스템이 또 "결정 한 줄"을 입력하라고 요구하고, 추천 한 줄 받기는 대표님 짧은 답만 25자로 압축 → 본질 손실.

fix 2종:
1) "그림 일치" 누르는 순간 마지막 라운드 Claude 답을 자동으로 textarea에 채움
2) "Claude 추천 한 줄 받기" 버튼도 마지막 Claude 답 사용 (대표님 답 압축 X)
3
