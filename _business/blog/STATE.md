...
> 최종 갱신: 2026-08-05 (독립 레포 분리 완료 세션)
...
## 실사이트 소스 (정본) ⭐
- **레포: `dgmasters01/staycurate` (Private)** — staycurate.com의 진짜 소스. 저장 시 `{"repo":"staycurate", ...}` 지정 필수.
  - `index.html`(홈) · `best-osaka-hotels/`(베스트목록) · `osaka-hotel-budget/`(예산분해) · `kyoto/`(카테고리)
- 배포: ✅ **자동 배포 가동 중** — Vercel `staycurate` ↔ GitHub `dgmasters01/staycurate`, Root Directory 비움. 커밋 1~2분 뒤 실사이트 반영(D-B19·D-B20).
- 라이브 페이지: `/`(홈) · `/best-osaka-hotels`(베스트7) · `/osaka-hotel-budget`(예산분해) · `/kyoto`(카테고리) — 전부 200 확인(2026-08-05)
