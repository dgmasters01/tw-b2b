...
## 🟢 2026-09-17 바뀐 것 — 먼저 읽는다

| 무엇 | 전 | 지금 |
|---|---|---|
| §4 넣기 SQL | 창구가 «값을 바꾸는 WITH»를 거부해 실행하는 쪽이 매번 3문장으로 쪼갰다 | 🟢 **창고 함수를 고쳐 §4 그대로 한 번에 실행된다** (그대로 붙여 넣는다 · 쪼개지 않는다) |
| §7 시작 줄 | `insert into shop_kk_log …` | 🟢 **`select shop_kk_run_open(<대상 수>) id`** — 오늘 줄이 있으면 그 줄을, 없으면 새로 만든다(하루 한 줄) |
| §7 끝 줄 | 실행 쪽이 빠뜨리면 점검이 «안 돌았다»로 알렸다(9/17 실제로 1,052개 받고도 빨강) | 🟢 **가격이 들어올 때마다 창고가 스스로 받은 수·못 받은 수·끝난 시각을 적는다**(방아쇠 `trg_shop_kk_log_*`). 80% 넘으면 `ok=true` |
| 실행 쪽이 할 일 | 시작·끝 두 줄 | 시작 한 줄 + **끝에 note 한 줄**(무슨 일이 있었는지). 실패면 `route='실패'`·`ok=false` — 이 줄은 창고가 덮지 않는다 |

🔴 기록을 빠뜨려도 **자료는 남는다** — 그래도 §7 은 지킨다. 실패 이유(403·노트북 꺼짐)는 사람만 적을 수 있다.
되돌리기·전말: `travelwinners-shop` 레포 `sql/2026-09-17-kk-log-auto.sql` · `_ref/2026-09-17-ROLLBACK.md` U절
...
🟢 **2026-09-17 부터 아래 SQL 이 그대로 실행된다** — 쪼개지 않는다. 결과로 `갱신` 수가 돌아온다.
...
**시작할 때** (2026-09-17 바뀜 — 오늘 줄이 이미 있으면 그 번호를 돌려준다)
```sql
select shop_kk_run_open(<N>) id
```

**끝날 때 (성공이든 실패든)**
🟢 받은 수·못 받은 수·끝난 시각은 **창고가 이미 적어 둔다.** 아래는 «어떻게 끝났는지» 사람만 아는 것을 덧붙이는 줄이다.
```sql
