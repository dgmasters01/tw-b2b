...
## 🔴 먼저 확인해야 할 것 — 예약 작업이 «어디서» 도는가 (2026-09-09 미해결)

Anthropic 지원 문서: **예약 작업은 «원격»에서 돌아, 컴퓨터가 잠들어 있거나 앱이 닫혀 있어도 실행된다.**

🔴 **그렇다면 그것은 「대표님 집 회선」이 아니라 「Anthropic 클라우드」다 — 즉 데이터센터 IP 다.**
Vercel(403)·GitHub Actions(403)와 **같은 종류**일 수 있다. 그러면 매일 403 만 쌓인다.

**이것은 «재보기 전에는 알 수 없다».** 클로드가 여기서 확인할 방법이 없다 — 그 클라우드에서 직접 눌러 봐야 한다.

| 첫 회차 결과 | 뜻 | 그러면 |
|---|---|---|
| 🟢 **200 · 값이 들어옴** | 클라우드 IP 가 KKday 를 통과한다 | **끝. 매일 저절로 돈다** |
| 🔴 **403** | 클라우드도 막힌다 | 아래 «막혔을 때» 로 간다 |

**🔴 막혔을 때 갈 길 (미리 정해 둔다 — 그때 헤매지 않게)**

| 안 | 내용 | 돈 |
|---|---|---|
| **A. 데스크톱 로컬 실행** | 예약 작업에 **로컬/원격 선택**이 있는지 본다. 로컬이면 대표님 집 회선이라 통과한다. 단 **노트북이 켜져 있어야** 한다 | 0원 |
| **B. 대표님이 한 줄 붙여넣기** | 매일이 아니라 **주 1회**로 낮추고, 대표님이 Cowork 에 직접 한 줄 넣는다. 값은 하루 0.14% 씩만 흔들려 일주일 밀려도 1% 안쪽이다 | 0원 |
| **C. Claude for Chrome** | 대표님 크롬에서 돌린다 — 확실히 집 회선이다 | 0원 |

🔴 **B 가 최소한의 보장선이다.** A·C 가 다 안 돼도 **주 1회는 반드시 굴러간다.**
