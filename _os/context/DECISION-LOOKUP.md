...
| **🔴 백업을 만질 때 (의무)** | 🔴 **먼저 `staycurate docs/BUSINESS-MAP.md §5-C 「백업과 복구」` 를 읽는다.** 거기에 «무엇을·어디에·왜 나눠서·어떻게 복구» 가 다 있다. 처음부터 다시 설계하지 마라 — 09-02 에 클로드가 문서를 안 찾아 구글 드라이브로 두 번 실패하고 대표님 시간을 썼다. 🔴 **저장처는 GitHub 이다**(git 델타가 바뀐 줄만 저장 — 30일 커밋해도 용량이 안 는다). 드라이브·R2 같은 «객체 저장»은 올릴 때마다 통째로 쌓여 백업에 부적합하다 | `GAPS BJ` 2026-09-02 |
